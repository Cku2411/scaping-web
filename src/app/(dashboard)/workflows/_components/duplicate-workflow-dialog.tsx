...
const DuplicateWorkflowDialog = ({ workflowId }: Props) => {
  ...
  const { duplicateWorkflowMutation, isPendingDuplicateWorkflow } =
    useDuplicateWorkflow();

  // form

  const form = useForm<DuplicateWorkflowSchemaType>({
    resolver: zodResolver(duplicateWorkFlowFormSchema),
    defaultValues: { workflowId, name: "", description: "" },
  });

  const onWorkflowFormSubmit = useCallback(
    (values: DuplicateWorkflowSchemaType) => {
      toast.loading("Duplicating workflow ...", { id: "duplicate-workflow" });
      duplicateWorkflowMutation(values, {
        onSuccess: () => {
          setOpen((prev) => !prev);
        },
      });
    },
    [duplicateWorkflowMutation, setOpen]
  );
          ...
          <Form {...form}>
            ...
            >
              ...
              <Button
                type="submit"
                className="w-full"
                disabled={isPendingDuplicateWorkflow}
              >
                {!isPendingDuplicateWorkflow ? (
                  "Duplicate"
                ) : (
